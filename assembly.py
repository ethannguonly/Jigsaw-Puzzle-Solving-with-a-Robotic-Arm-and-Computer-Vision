...
for x in range(0, 4):
    ...
    out.save("piece"+str(x+1)+".png", "PNG")

for num in range(0, 4):
    canvas = cv2.imread('Pirateref.jpg')
    piece = cv2.imread('piece'+str(num+1)+'.png')
    ...
    if len(good) > MIN_MATCH_COUNT:
        ...
        print(int((num%puzzlewidth+0.5)*width/puzzlewidth))
        print(int((int(num/puzzlewidth)+0.5)*height/puzzleheight))
        print(xavg)
        print(yavg)
    else:
